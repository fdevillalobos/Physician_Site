...
Scenario: View doctors in database
    Given doctors are in the database
    Given I'm on the main doctor list webpage
...
Scenario: Search for existing doctors
    ...
    Then I should see at least one doctor result
...
Scenario: Search for non-existing doctors
  ...
  Scenario: Sort doctors by distance ascending
    Given I've searched for a doctor that returns many results
